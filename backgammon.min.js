...
var canclear=self.canclear();if(canclear!==false){var clearfrom=self.wtm?0:canclear;var clearto=self.wtm?canclear:23;for(var point=clearfrom;point<=clearto;point++){if(!roll.doubles){if(clearto-point<=roll.die1){var possible=new Move(die1,point,999,false);moves.push(possible);}
if(clearto-point<=roll.die2){var possible=new Move(die2,point,999,false);moves.push(possible);}}else{if(clearto-point<=roll.die1){for(var i=0;i<4;i++){var possible=new Move(die1,point,999,false);moves.push(possible);}}}}}
...
html+='<td class="backgammon-right"> </td></tr>';html+='</table></div>'
